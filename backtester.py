...
# --- 商品クラス別シグナルゲート ---
# results/signals_history.csv の 59 営業日 / 245 onset (2026-04〜07) を
# フォワードリターンで検証して特定した、構造的な負けパターンの遮断ルール。
# 詳細は docs/signal_accuracy_2026-07.md を参照。

TICKER_CLASS_PLAIN = "plain"            # 現物個別株・非レバレッジETF
TICKER_CLASS_LONG_LEV = "long_lev"      # ロング型レバレッジETF (TQQQ/SOXL等)
TICKER_CLASS_INVERSE_LEV = "inverse_lev"  # インバース型レバレッジETF (SQQQ/SOXS等)


def resolve_ticker_class(ticker, config):
    """config.toml の [ticker_classes] からティッカーの商品クラスを引く。
    未定義のティッカーは plain 扱い。"""
    classes = (config or {}).get("ticker_classes", {})
    for cls, tickers in classes.items():
        if ticker in tickers:
            return cls
    return TICKER_CLASS_PLAIN


def compute_signal_gates(data, ticker_class=TICKER_CLASS_PLAIN):
    """新規シグナル発火の可否ゲートを計算する (bool 配列の dict を返す)。

    実測で特定した負けパターンの遮断 (数値は 10 営業日後の勝率 / 平均):
...
def calculate_indicators_and_signals(data_hash, _data, params, strategy_type="トレンドフォロー"):
    ...
    try:
        ...
        data['ema200'] = _ema(data['close'], length=200)
        ...
        if strategy_type == "トレンドフォロー":
            ...
            # --- 方針5: 落ちるナイフ回避フィルター（逆張りBUYの精度改善）---
            # 長期構造が崩れた銘柄（インバースETFの長期減価等）の押し目買いは
            # 実測で10日後勝率0%・平均-18%。長期トレンドが生きている場合のみ買う。
            long_ma_rising_c = data['sma_long'] > data['sma_long'].shift(10)
            long_trend_ok = (data['close'] > data['ema200']) | long_ma_rising_c
            # 乖離が深すぎる/RSIが極端に低い = 暴落モメンタム中（ナイフ）
            falling_knife = (data['deviation'] < -15) | (data['rsi'] < 20)
            # 反発の初動確認（陽転 or RSI上向き）が無い限りエントリーしない
            rebound_confirm = (data['close'] > data['close'].shift(1)) | (data['rsi'] > data['rsi'].shift(1))

            # 売られすぎ極値の当日は下落中であることが多いため、
            # 「直近3日以内にスコアが買い閾値に達した + 当日反発確認」でエントリー
            ...
            # MACDクロス単独では発火させず、価格が短期SMAを割っていることを必須にする
            # （上昇トレンド中の一時的なモメンタム鈍化でのダマシ売りを防ぐ）
            macd_cross_down_c = (data['macdh'] < 0) & (data['macdh'].shift(1) >= 0)

            early_sell_signal_c = (profit_take_signal | macd_cross_down_c) & (data['close'] < data['sma_short'])

            # --- 方針6: 強い上昇トレンド中の逆張り売り禁止 ---
            # 長期EMAから+5%超乖離して上昇継続中の銘柄への過熱売りは
            # 実測で10日後勝率21%・平均-14%（踏み上げられる）。
            strong_uptrend_block = uptrend_momentum & (data['deviation'] > 5)
            ...
            # 落ちるナイフ回避フィルター（逆張りパートにも適用）
            lmr_cr = data['sma_long'] > data['sma_long'].shift(10)
            lt_ok_cr = (data['close'] > data['ema200']) | lmr_cr
            knife_cr = (data['deviation'] < -15) | (data['rsi'] < 20)
