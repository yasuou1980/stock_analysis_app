...
def calculate_indicators_and_signals(data_hash, _data, params, strategy_type="トレンドフォロー"):
    ...
    try:
        ...
        # pandas-taの仕様に対応するため、戻り値を明示的に代入
        ...
        # ADX (トレンド強度) の計算 (これは内部で複数カラムを返すため動的取得でOK)
        ...
        if strategy_type == "トレンドフォロー":
            ...
            # 2. RSI ゾーンスコア: 状態確認として低重みに (±0.5)
            # 「状態」ではなく「転換」を重視するためスコアを抑える
            rsi_zone_score = np.select([data['rsi'] > 50, data['rsi'] < 50], [0.5, -0.5], default=0)
            ...
            # 6. MACDゼロライン越え: トレンド開始・終了の最重要シグナル ±2.0
            # ゼロライン上抜け = 下降→上昇トレンドへの転換(底反転後の勢い確認)
            # ゼロライン下抜け = 上昇→下降トレンドへの転換(天井後の崩れ確認)
            macd_zero_cross_up   = (data['macd'] > 0) & (data['macd'].shift(1) <= 0)
            macd_zero_cross_down = (data['macd'] < 0) & (data['macd'].shift(1) >= 0)
            macd_zero_cross_score = np.select([macd_zero_cross_up, macd_zero_cross_down], [2.0, -2.0], default=0)

            # 7. RSI 底・天井からの反転確認 ±2.0
            # RSI35上抜け: 売られすぎ圏から回復 → 底反転BUYの確認
            # RSI65下抜け: 買われすぎ圏から反落 → 天井手前SELLの早期検出
            rsi_cross_up35   = (data['rsi'] > 35) & (data['rsi'].shift(1) <= 35)
            rsi_cross_down65 = (data['rsi'] < 65) & (data['rsi'].shift(1) >= 65)
            rsi_reversal_score = np.select([rsi_cross_up35, rsi_cross_down65], [2.0, -2.0], default=0)

            # 8. ベアリッシュダイバージェンス: 天井手前のSELLシグナル -1.5
            # 価格が直近高値付近にいるのにMACDヒストグラムが前回ピークより失速
            # → 上昇モメンタム低下 = 天井形成中のシグナル
            ...
            # 9. スコア合算
            # EMAステート: 背景確認として低重み ±0.5 (「状態」依存をなくす)
            # 転換シグナル (MACD零越え, RSI反転) を高重みで追加して「転換点」エントリーを実現
            # 最大BUYスコア: 0.5+0.5+1.5+1.5+1.0+1.0+0.5+2.0+2.0 = 10.5
            # 最大SELLスコア: -(同上+1.5ダイバージェンス) = -12.0
            ...
            # Numpy配列に変換して論理演算エラーを回避
            if 'volume' in data.columns:
                vol_condition = (data['volume'] > data['vol_sma'] * 1.5).values
            else:
            ...
            # BUY: 6.0以上 → 転換シグナルが1つ以上必要 (純粋な「状態維持」では届かない)
            # SELL: -5.5以下 → 天井付近の失速・反転を確認してから売り
            buy_signal  = (scores >= 6.0) & vol_condition & mtf_buy_filter
            sell_signal = (scores <= -5.5) & vol_condition & mtf_sell_filter
