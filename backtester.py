...
def calculate_indicators_and_signals(data_hash, _data, params, strategy_type="トレンドフォロー"):
    ...
    try:
        ...
        if strategy_type == "トレンドフォロー":
            ...
            # トレンドフォローは「売り遅れ」を防ぐため、価格とSMAの関係を重視して素早く逃げる
            ...
            # 3. パラボリックSARの陰転（トレンド下落サイン）
            psar_bearish_cross = (data['close'] < data['psar']) & (data['close'].shift(1) >= data['psar'].shift(1))

            # 即売り条件（いずれかを満たせば SELL シグナル）
            # A: 短期SMAを下回り、かつSMAが下向き（トレンドの完全な失速）
            fast_exit_cond_A = price_below_sma & sma_slope_negative
            # B: MACDデッドクロス かつ PSAR陰転（モメンタムの急低下）
            fast_exit_cond_B = macd_cross_down & psar_bearish_cross
            # C: 単純に短期SMAを割り込み、かつRSIが60未満（勢いがない状態でのサポート割れ）
            fast_exit_cond_C = price_below_sma & (data['rsi'] < 60)

            data['exit_signal'] = np.where(fast_exit_cond_A | fast_exit_cond_B | fast_exit_cond_C, "SELL", "HOLD")
    ...
    for i in range(len(_data)):
        ...
        if portfolio['shares'] > 0 and current_entry_price > 0:
            ...
            if current_atr > 0:
                ...
                # 含み益が出ても、ある程度は押し目を許容してトレンドを伸ばす設定
                if pnl_pct > 0.20:     # 含み益が20%を超えたら
                    dynamic_trailing_mult = 2.0  # ATRの2.0倍までストップを近づける（利益確保）
                elif pnl_pct > 0.10:   # 含み益が10%を超えたら
                    dynamic_trailing_mult = 2.5  # ATRの2.5倍
                else:
                    dynamic_trailing_mult = trailing_atr_mult # 初期値 (通常はATRの3.0倍)
