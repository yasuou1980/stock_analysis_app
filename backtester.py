    ...
    try:
        ...
        if strategy_type == "トレンドフォロー":
            ...
            if adx_col:
                ...
                # レンジ相場フィルタ: ADX < 20 のときスコアを半減してダマシを抑制
                adx_trend_filter = (data[adx_col] >= 20).values
            ...
            # 5. スコア合算 (最大 ±7.0、クロス発生日は ±8.0)
            ema_state_score = np.where(data['sma_short'] > data['sma_long'], 2, -2)
            macd_score      = np.where(data['macdh'] > 0, 1.5, -1.5)
            scores = ema_state_score + rsi_score + macd_score + adx_score + cross_bonus + breakout_score

            # ADX < 20 のレンジ相場ではスコアを半減
            ...
            # BUY: 5.0以上 (強いトレンド確認が必要)
            # SELL: -3.5以下 (押し目程度では売らない、出来高フィルタも適用)
            buy_signal  = (scores >= 5.0) & vol_condition
            sell_signal = (scores <= -3.5) & vol_condition
