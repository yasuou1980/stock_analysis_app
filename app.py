...
@st.cache_data(ttl=3600)  # 1時間のTTLを追加
def load_data(ticker, start_date_str, end_date_str):
    """Yahoo Financeから株価データを読み込む - 修正版"""
    try:
...
def calculate_indicators_and_signals(data_hash, _data, params):
    ...
    try:
        ...
        data['composite_signal'] = np.select(
            [scores >= 3.5, scores <= -3.5], 
            ["BUY", "SELL"], 
            default="HOLD"
        )
...
@st.cache_data
def backtest_strategy(data_hash, _data, initial_capital, commission_rate, slippage):
    """バックテスト実行 - ハッシュ値で強制更新"""
    ...
    for i in range(len(_data)):
        ...
        if row['composite_signal'] == "BUY" and portfolio['shares'] == 0:
            ...
            trades.append({
                'date': _data.index[i], 'action': 'SELL', 
...
def calculate_performance_metrics(_portfolio_values, _dates):
    ...
    return {
        'total_return': total_return, 'max_drawdown': max_dd, 'sharpe_ratio': sharpe, 
...
def calculate_trade_metrics(trades_df):
    ...
    hold_periods = [(sells.iloc[i]['date'] - buys.iloc[i]['date']).days 
...
# ティッカー選択（セッション状態で管理）
current_ticker_index = TICKERS.index(st.session_state.ticker) if st.session_state.ticker in TICKERS else 0
ticker_choice = st.sidebar.selectbox("ティッカー", TICKERS, index=current_ticker_index)
...
preset_choice = st.sidebar.selectbox(
    "設定プリセット", 
    list(PRESETS.keys()), 
    index=list(PRESETS.keys()).index(st.session_state.preset_choice)
)
...
params = {}
for key, (label, min_val, max_val, def_val) in params_config.items():
    current_val = st.session_state.params.get(key, def_val)
...
if st.sidebar.checkbox("デバッグ情報を表示"):
    ...
    st.sidebar.write(f"**選択中のティッカー**: `{ticker_choice}`")
    st.sidebar.write(f"**セッション状態のティッカー**: `{st.session_state.ticker}`")
...
if raw_data is None or not validate_date_range(start_date, end_date): 
    st.stop()
...
with st.spinner('バックテストを実行中...'):
    results_hash = hash(str(data.values.tobytes()) + str(initial_capital) + str(commission_rate) + str(slippage))
    results = backtest_strategy(results_hash, data, initial_capital, commission_rate, slippage)
...
if metrics:
    ...
    with tab1:
        ...
        portfolio_df = pd.DataFrame(
            {'戦略ポートフォリオ': results['portfolio_values']}, 
            index=results['dates']
        )
    ...
    with tab2:
        ...
        c3.metric("歪度 (Skewness)", f"{metrics.get('skewness', 0):.2f}", 
                 help="0より大きいと右に長い裾野（大きな利益が時々）、小さいと左に長い裾野（大きな損失が時々）")
        c4.metric("尖度 (Kurtosis)", f"{metrics.get('kurtosis', 0):.2f}", 
                 help="3より大きいと正規分布より尖った分布（テールリスク大）")
    ...
    with tab3:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                mode='lines', name=f'SMA {params["short_window"]}'
            ), 
            row=1, col=1
        )
        fig.add_trace(
            go.Scatter(
                ...
                mode='lines', name=f'SMA {params["long_window"]}'
            ), 
            row=1, col=1
        )
        ...
        fig.add_trace(
            go.Bar(
                x=data.index, y=data['macdh'], 
                name='MACDヒストグラム', 
                marker_color=np.where(data['macdh'] > 0, 'green', 'tomato')
            ), 
            row=2, col=1
        )
