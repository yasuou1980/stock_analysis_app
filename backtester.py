...
def calculate_indicators_and_signals(data_hash, _data, params, strategy_type="トレンドフォロー"):
    ...
    try:
        ...
        if strategy_type == "トレンドフォロー":
            ...
            # --- エグジット専用シグナル（ポジション保有時の早期売り判定）---
            ema_slope_series = pd.Series(ema_slope, index=data.index)
            exit_cond_slope = (ema_slope <= 0) & (ema_slope_series.shift(1) > 0)
            exit_cond_rsi = (data['rsi'] < 65) & (data['rsi'].shift(1) >= 65)
            macdh_s = data['macdh']
            exit_cond_macdh = (macdh_s < macdh_s.shift(1)) & (macdh_s.shift(1) < macdh_s.shift(2)) & (macdh_s > 0)
            exit_cond_below_ema = (data['close'] < data['sma_short']) & (data['close'].shift(1) >= data['sma_short'].shift(1))
            exit_cond_psar = psar_bearish_cross

            # ▼ 新規追加: プライスアクション（ローソク足の形状）による弱気サイン
            body_prev = data['close'].shift(1) - data['open'].shift(1)
            body_curr = data['close'] - data['open']
            # 1. ベアリッシュ・エンゴルフィング（弱気の包み足: 前日の陽線を完全に包み込む陰線）
            is_bearish_engulfing = (body_prev > 0) & (body_curr < 0) & \
                                   (data['open'] > data['close'].shift(1)) & \
                                   (data['close'] < data['open'].shift(1))
            
            # 2. 長い上ヒゲ（ピンバー: 高値圏での強い売り圧力を示す）
            upper_shadow = data['high'] - data[['open', 'close']].max(axis=1)
            total_range = data['high'] - data['low']
            is_long_upper_shadow = (upper_shadow > total_range * 0.5) & (total_range > data['atr'] * 0.8)

            exit_count = (exit_cond_slope.astype(int) + exit_cond_rsi.astype(int)
                          + exit_cond_macdh.astype(int) + exit_cond_below_ema.astype(int)
                          + exit_cond_psar.astype(int)
                          + is_bearish_engulfing.astype(int) 
                          + is_long_upper_shadow.astype(int))

            # ▼ 新規追加: ATR（ボラティリティ）ベースの動的閾値
            # 最近のボラティリティが急増している（相場が荒れている）場合は、通常3つの条件を「2つ」に緩和して素早く逃げる
            vol_increase = data['atr'] > data['atr'].rolling(window=10).mean() * 1.2
            exit_threshold = np.where(vol_increase, 2, 3)

            data['exit_signal'] = np.where(exit_count >= exit_threshold, "SELL", "HOLD")
            ...
            # ▼ 新規追加: 逆張り特化の早期利益確定シグナル
            # 1. ボリンジャーバンドの+2σにタッチして陰線で終わった場合
            bb_touch_reversal = (data['high'] >= data['bbu']) & (data['close'] < data['open'])
            # 2. RSIが過熱圏（設定値）に突入後、下抜けした場合
            rsi_overbought_reversal = (data['rsi'] < rsi_upper) & (data['rsi'].shift(1) >= rsi_upper)
            early_sell_signal_c = bb_touch_reversal | rsi_overbought_reversal
...
def backtest_strategy(data_hash, _data, initial_capital, commission_rate, slippage, position_sizing_strategy, ps_params):
    ...
    trailing_stop_price = 0     # ATRベースのトレーリングストップ価格
    ...
    for i in range(len(_data)):
        ...
        if portfolio['shares'] > 0 and current_entry_price > 0:
            ...
            if current_atr > 0:
                # ▼ 新規追加: 含み益に応じたストップ幅の動的調整
                pnl_pct = (highest_price_since_entry - current_entry_price) / current_entry_price
                if pnl_pct > 0.15:     # 含み益が15%を超えたら
                    dynamic_trailing_mult = 1.5  # ATRの1.5倍までストップを引き上げ
                elif pnl_pct > 0.05:   # 含み益が5%を超えたら
                    dynamic_trailing_mult = 2.0  # ATRの2.0倍までストップを引き上げ
                else:
                    dynamic_trailing_mult = trailing_atr_mult # 初期値 (3.0倍)
        ...
        if row['composite_signal'] == "BUY":
            ...
            trailing_stop_price = 0
            pyramid_count = 0
        elif portfolio['shares'] > 0 and 'exit_signal' in _data.columns and row.get('exit_signal') == "SELL":
            # エグジットシグナルによる早期売り（トレンドフォロー用）
            # 含み益が3%以上ある場合のみ適用（発展途上のポジションを早期に切らない）
